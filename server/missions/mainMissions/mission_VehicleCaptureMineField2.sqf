...
_setupObjects =
{
	...
	for "_count" from 1 to 20 do {
		_minePos = [ (_missionPos select 0) + ((random 100) - 50),(_missionPos select 1) + ((random 100) - 50), 0 ];
	...
	for "_i" from 0 to 359 step 45 do
		...
		_sign setDir (180+_i);
		_sign setPosATL (_missionPos getPos [52, _randDirOffset+_i]);
...
// Remove AT Minefield
_missionLoc = markerPos _missionLocation;
_nearestMines = nearestObjects [_missionLoc, ["All"], 50];
{ deleteVehicle _x } forEach _nearestMines;
//_nearestMines = _missionPos nearObjects ["TimeBombCore",50];
//	_nearestMines = _missionPos nearObjects ["ATMine",50];
//{deleteVehicle _x;} forEach _nearestMines; 
Hint "Mines Deleted";
// Remove AT Minefield
